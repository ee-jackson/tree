...
We noticed that the distribution of intial soil carbon didn't seem to change 
...
read.csv(
  here::here("data", "raw", "ForManSims_2016_RCP0.csv"),
  sep = ";", skipNul = TRUE) %>% 
  filter(period == 0 & peat == 0) %>% 
  ggplot() + 
  geom_point(aes(y = TotalSoilCarbon, x = Altitude))
